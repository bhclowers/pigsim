...
-- mods based on symmetry
-- better electrode dict reading next iteration, promise!
local sym = Symmetry
print(sym)
if sym == "ASY" 
then 
  mir = "''"
  push_line = string.format("adj_elect[%s] = _PushV", e_dict["P"])
else
  mir = "'z'"
  push_line = "\n"
end

local gem = string.format([[
...
------------------------------------------------------------------------------------------------------------------
---- Geometry ------------------------- symmetric or asymmetric, depending
------------------------------------------------------------------------------------------------------------------

pa_define{wx=math.floor(lt)+1,wy=math.floor(lty)+1,wz=math.floor(d*2)+2,dx=scale,mirror=%s,surface='fractional'}
...
-- there was a .fly2 generation here, I generate them with python now - CG
...
adjustable _frequency_hz       = 0.9E6  
...
]], e_dict["RF1"], e_dict["RF2"], e_dict["G"], push_line)
